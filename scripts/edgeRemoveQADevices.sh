...
az iot hub query --hub-name $iothub_name --query-command "SELECT * FROM devices WHERE tags.environment = '$environment'" | jq -r .[].deviceId | xargs -L 1 az iot hub device-identity delete --hub-name $iothub_name --device-id
